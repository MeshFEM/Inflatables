...
namespace DDG
{
   class Mesh
   {
      public:
         ...
         void import(const std::vector<Vertex > &vertices,
                     const std::vector<Element> &elements) {
            DDG::MeshData data;
            data.positions.reserve(vertices.size());
            for (const auto &v : vertices)
                data.positions.emplace_back(v[0], v[1], v[2]);
            data.indices.reserve(elements.size());
            for (const auto &e : elements) {
                if (e.size() != 3) throw std::runtime_error("Only triangle meshes supported");
                data.indices.emplace_back(std::vector<DDG::Index>{DDG::Index(e[0], -2, -2),
                                                                  DDG::Index(e[1], -2, -2),
                                                                  DDG::Index(e[2], -2, -2)});
            }
            initialize(data);
         ...
         template<typename VerticesEigen, typename ElementsEigen>
         std::enable_if_t<(VerticesEigen::ColsAtCompileTime == 3) &&
                          (ElementsEigen::ColsAtCompileTime == 3)>
         import(const VerticesEigen &vertices, const ElementsEigen &elements) {
            DDG::MeshData data;
            const size_t nv = vertices.rows(), ne = elements.rows();
            data.positions.reserve(nv);
            for (size_t i = 0; i < nv; ++i)
                data.positions.emplace_back(vertices(i, 0), vertices(i, 1), vertices(i, 2));
            data.indices.reserve(ne);
            for (size_t i = 0; i < ne; ++i) {
                data.indices.emplace_back(std::vector<DDG::Index>{DDG::Index(elements(i, 0), -2, -2),
                                                                  DDG::Index(elements(i, 1), -2, -2),
                                                                  DDG::Index(elements(i, 2), -2, -2)});
            }
            initialize(data);
         }
